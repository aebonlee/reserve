...
import type { Reservation, ReservationUserData, ReservationStats } from '../types';

/**
 * 예약 생성
 */
export const createReservation = async (scheduleId: string, userData: ReservationUserData): Promise<Reservation> => {
...
export const cancelReservation = async (id: string): Promise<Reservation> => {
  ...
  // current_count 감소
  if (reservation) {
    await client.rpc('decrement_schedule_count', { schedule_id: (reservation as { schedule_id: string }).schedule_id });
  }
 ...
 * 일정별 예약 (관리자)
 */
export const getReservationsBySchedule = async (scheduleId: string): Promise<Reservation[]> => {
  const client = getSupabase();
  if (!client) return [];
 ...
 */
export const checkExistingReservation = async (scheduleId: string, userId: string): Promise<Reservation | null> => {
  const client = getSupabase();
  ...
  const rows = data as Reservation[] | null;
