...
function toCamel(row: Record<string, unknown> | null): SearchResultItem | null {
  ...
  return out as SearchResultItem;
}
...
export async function searchAll(query: string): Promise<SearchResults> {
  ...
  return {
    blog: ((blogRes.data ?? []) as Array<Record<string, unknown>>).map(toCamel).filter((x): x is SearchResultItem => x !== null),
    board: ((boardRes.data ?? []) as Array<Record<string, unknown>>).map(toCamel).filter((x): x is SearchResultItem => x !== null),
    gallery: ((galleryRes.data ?? []) as Array<Record<string, unknown>>).map(toCamel).filter((x): x is SearchResultItem => x !== null)
  };
}
