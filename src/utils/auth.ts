...
/** Kakao OAuth 로그인 */
export async function signInWithKakao(): Promise<unknown> {
  const client = getSupabase();
  if (!client) throw new Error('Supabase not configured');
  const { data, error } = await client.auth.signInWithOAuth({
    provider: 'kakao',
...
/** 이메일 회원가입 */
export async function signUp(email: string, password: string, displayName: string): Promise<unknown> {
  const client = getSupabase();
  if (!client) throw new Error('Supabase not configured');
...
/** 프로필 업데이트 */
export async function updateProfile(userId: string, updates: ProfileUpdate): Promise<UserProfile | null> {
  const client = getSupabase();
  if (!client) return null;
  const { data, error } = await client
    .from('user_profiles')
