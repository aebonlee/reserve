...
import type { LectureRequestFormState, LectureRequestFormData } from '../../types';

interface LectureRequestFormProps {
  onSubmit: (data: LectureRequestFormData) => void;
  loading: boolean;
}

const LectureRequestForm = ({ onSubmit, loading }: LectureRequestFormProps): React.ReactElement => {
  const { t } = useLanguage();
  const [form, setForm] = useState<LectureRequestFormState>({
    requester_name: '',
    requester_email: '',
    requester_phone: '',
  ...
  const handleDatesChange = (dates: string[]): void => {
