...
import type { Schedule as ScheduleType, DailyHoursMap } from '../types';
...
const Schedule = (): React.ReactElement => {
  ...
  const [selectedDateSchedules, setSelectedDateSchedules] = useState<ScheduleType[] | null>(null);
  ...
  const handleDateClick = (dateStr: string, daySchedules: ScheduleType[]): void => {
        ...
        <div className="container">
          <div className="schedule-toolbar">
            <div className="view-toggle">
              ...
              <button className={`view-toggle-btn${viewMode === 'list' ? ' active' : ''}`} onClick={() => setViewMode('list')}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><line x1="8" y1="6" x2="21" y2="6" /><line x1="8" y1="12" x2="21" y2="12" /><line x1="8" y1="18" x2="21" y2="18" /><line x1="3" y1="6" x2="3.01" y2="6" /><line x1="3" y1="12" x2="3.01" y2="12" /><line x1="3" y1="18" x2="3.01" y2="18" /></svg>
                {t('schedule.listView')}
              </button>
            </div>
            <div className="category-filters">
              {categories.map(cat => (
                <button key={cat} className={`category-filter-btn${categoryFilter === cat ? ' active' : ''} ${cat !== 'all' ? getCategoryClass(cat) : ''}`} onClick={() => setCategoryFilter(cat)}>
          ...
          {loading ? (
            ...
            <div className="calendar-layout">
              <CalendarView schedules={filteredSchedules} year={year} month={month} onMonthChange={handleMonthChange} onDateClick={handleDateClick} dailyHours={dailyHours} />
              {selectedDateSchedules !== null && (
                <div className="calendar-day-schedules">
                  <h4>{selectedDate}</h4>
                  {selectedDateSchedules.length > 0 ? (
                    <div className="schedule-list">{selectedDateSchedules.map(s => (<ScheduleCard key={s.id} schedule={s} />))}</div>
                  ) : (
            ...
            <div className="schedule-grid">
              {filteredSchedules.length > 0 ? filteredSchedules.map(s => (<ScheduleCard key={s.id} schedule={s} />)) : (<div className="empty-state"><p>{t('schedule.noSchedules')}</p></div>)}
            </div>
          )}
        </div>
      </section>
