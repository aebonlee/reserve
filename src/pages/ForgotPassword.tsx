        ...
        <div className="auth-card-google">
          ...
          {sent ? (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ fontWeight: 600, marginBottom: '8px', color: 'var(--text-primary)' }}>{t('auth.resetEmailSent')}</p>
              <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '24px' }}>{t('auth.checkEmailForReset')}</p>
              <Link to="/login" className="auth-next-btn" style={{ display: 'inline-block', textDecoration: 'none', textAlign: 'center' }}>{t('auth.backToLogin')}</Link>
            </div>
          ) : (
            <form onSubmit={(e) => void handleSubmit(e)} className="auth-email-form">
              ...
              <div className="auth-form-actions">
                <Link to="/login" className="auth-back-btn" style={{ textDecoration: 'none', textAlign: 'center' }}>{t('auth.backToLogin')}</Link>
                <button type="submit" className="auth-next-btn" disabled={loading}>{loading ? t('auth.sending') : t('auth.sendResetLink')}</button>
              </div>
            </form>
          )}
        </div>
