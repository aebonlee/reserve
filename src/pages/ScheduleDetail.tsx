...
import type { Schedule, Reservation, ReservationUserData } from '../types';
...
const ScheduleDetail = (): React.ReactElement => {
  ...
  const handleReservation = async (formData: ReservationUserData): Promise<void> => {
    if (!id) return;
    setReserving(true);
    try {
      ...
      showToast((err as Error).message || t('reservation.error'), 'error');
    } finally {
      setReserving(false);
    }
  };
  ...
  const formatTime = (time: string | null): string => time ? time.slice(0, 5) : '';
  ...
  const remaining = schedule.capacity > 0 ? schedule.capacity - (schedule.current_count ?? 0) : null;
              ...
              <div className="reservation-card">
                ...
                {!isLoggedIn ? (
                  ...
                  <div className="reservation-done">
                    ...
                    <Link to="/my-reservations" className="btn btn-secondary mt-2">{t('reservation.viewMyReservations')}</Link>
                  </div>
                ) : !canReserve ? (
                  ...
                  <ReservationForm onSubmit={(data) => void handleReservation(data)} loading={reserving} />
                )}
              </div>
            </div>
          </div>
