...
const MyReservations = (): React.ReactElement => {
  ...
  const handleCancel = async (reservationId: string): Promise<void> => {
    ...
    try {
      ...
      showToast((err as Error).message || t('reservation.cancelError'), 'error');
    }
  };
  ...
  const formatTime = (time: string | undefined): string => time ? time.slice(0, 5) : '';
          ...
          {loading ? (
            ...
            <div className="reservations-list">
              {filtered.map(r => {
                ...
                const isPast = schedule ? new Date(schedule.date + 'T' + (schedule.end_time ?? '23:59')) < now : false;
                return (
                  <div key={r.id} className={`reservation-item status-${r.status}${isPast ? ' past' : ''}`}>
                    ...
                    {r.status === 'confirmed' && !isPast && (
                      <div className="reservation-item-actions">
                        <button className="btn-cancel" onClick={() => void handleCancel(r.id)}>
