            ...
            <div className="lr-success">
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                <button className="btn btn-primary" onClick={() => setSubmitted(false)}>
                  {t('lectureRequest.submitAnother')}
                </button>
                <Link to="/lecture-request" className="btn btn-secondary">
                  {t('lectureRequest.backToList')}
                </Link>
              </div>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <section className="section">
        <div className="container">
          {error && (
            <div style={{ color: '#EF4444', textAlign: 'center', marginBottom: '16px', fontSize: '14px' }}>
              {error}
            </div>
          )}
          <LectureRequestForm onSubmit={handleSubmit} loading={loading} />
        </div>
      </section>
    </>
  );
