...
import type { Schedule, Reservation, ScheduleFormData } from '../../types';

const ScheduleManage = (): React.ReactElement => {
  ...
  const [viewReservations, setViewReservations] = useState<string | null>(null);
  ...
  const handleCreate = async (formData: ScheduleFormData): Promise<void> => {
    try {
      await createSchedule({ ...formData, created_by: user?.id ?? null });
      showToast(t('admin.scheduleCreated'), 'success');
      setShowForm(false);
      void loadSchedules();
    } catch (err) {
  ...
  const handleUpdate = async (formData: ScheduleFormData): Promise<void> => {
    if (!editingSchedule) return;
    try {
      await updateSchedule(editingSchedule.id, formData);
      showToast(t('admin.scheduleUpdated'), 'success');
  ...
  const handleDelete = async (id: string): Promise<void> => {
    ...
    try {
      ...
      showToast(t('admin.scheduleDeleted'), 'success');
      void loadSchedules();
    } catch (err) {
      showToast((err as Error).message, 'error');
    }
  ...
  const handleViewReservations = async (scheduleId: string): Promise<void> => {
          ...
          {(showForm || editingSchedule) && (
            <div className="admin-form-wrapper">
              ...
              <ScheduleForm
                ...
                onSubmit={editingSchedule ? (data) => void handleUpdate(data) : (data) => void handleCreate(data)}
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="admin-row-actions">
                            ...
                            <button className="admin-row-btn" onClick={() => void handleViewReservations(s.id)}>
                              {t('admin.viewReservations')}
                            </button>
                            <button className="admin-row-btn danger" onClick={() => void handleDelete(s.id)}>
                              {t('common.delete')}
                            </button>
                          </div>
                        </td>
