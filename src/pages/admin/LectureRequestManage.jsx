...
const LectureRequestManage = () => {
  ...
  const handleApprove = async (scheduleData) => {
  ...
  const filters = [
    { value: null, label: t('lectureRequest.statusAll'), count: stats.total },
    { value: 'pending', label: t('lectureRequest.statusPending'), count: stats.pending },
    { value: 'approved', label: t('lectureRequest.statusApproved'), count: stats.approved },
    { value: 'rejected', label: t('lectureRequest.statusRejected'), count: stats.rejected }
  ];

  const buildPrefilledSchedule = (req) => ({
    title: req.title || '',
        ...
        <div className="lr-modal-overlay" onClick={() => !processing && setRejectTarget(null)}>
          <div className="lr-modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-primary" onClick={handleReject} disabled={processing} style={{ background: '#EF4444', borderColor: '#EF4444' }}>
...
const TableRow = ({ req, expanded, onToggle, onApprove, onReject, onDelete, formatDate, t, language }) => {
